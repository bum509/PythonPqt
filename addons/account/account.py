...
#                if can_compute:
    ...
    _columns = {
        #Edit
        'x_account_type': fields.selection([('debit', 'Debit'), ('credit', 'Credit'), ('both', 'Both')], 'Debit/Credit'),
...
class account_journal(osv.osv):
    ...
    _columns = {
        ...
        #Edit
        'type': fields.selection([('sale', 'Sale'),('sale_refund','Sale Refund'), ('purchase', 'Purchase'), ('purchase_refund','Purchase Refund'), ('cash', 'Cash'), ('bank', 'Bank and Cheques'), ('general', 'General'), ('situation', 'Opening/Closing Situation'), ('expense', 'Expense'), ('payment', 'Payment')], 'Type', size=32, required=True,
		...
		'x_active':fields.boolean('Active'), #set journal co dung hay khong?
    }
    ...
    def _check_currency(self, cr, uid, ids, context=None):
        for journal in self.browse(cr, uid, ids, context=context):
            if journal.currency:
                if (journal.default_credit_account_id.currency_id.id and journal.default_credit_account_id.currency_id.id != journal.currency.id)\
                or (not journal.default_credit_account_id.currency_id.id and journal.currency.id != journal.company_id.currency_id.id): 
                        return False
                if (journal.default_debit_account_id.currency_id.id and journal.default_debit_account_id.currency_id.id != journal.currency.id)\
                or (not journal.default_debit_account_id.currency_id.id and journal.currency.id != journal.company_id.currency_id.id):
                    return False
        return True
...
class account_period(osv.osv):
    ...
    _order = "fiscalyear_id desc, date_start, special desc"
    _sql_constraints = [
...
class account_move(osv.osv):
    ...
    def _default_journal_id(self, cr, uid, context=None):
        ...
        if journal_type == 'expense':   
            ...
            if ids:
                journal_id = ids[0]
        #else:
        #   ids = journal_pool.search(cr, uid, [('company_id','=',company_id)])
        #    if ids:
        #        journal_id = ids[13]
        return journal_id

    _columns = {
        'name': fields.char('Number', size=64, required=True, states={'posted':[('readonly',True)]}),
        'ref': fields.char('Reference', size=64, states={'posted':[('readonly',True)]}),
        ...
        #Edit
        'x_amount_total': fields.float('Amount Total'),
        #'x_contract_id': fields.many2one('account.analytic.account', 'Contract Number', ondelete="cascade"),
        'x_expense_of': fields.many2one('x_expense.recurring', 'Expense of', required=False, select=True, readonly=True, states={'draft':[('readonly',False)]}),
		'x_number': fields.related('line_id','x_number', string = 'Invoice Number',type='char', size=25, readonly=True),
    }
    ...
    def post(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, valid_moves, context=context):
            #check over budget on real cash
            if move.journal_id.type == 'expense': 
                for obj_line in move.line_id:
                     if obj_line.analytic_account_id.code=='TM' and obj_line.credit > 0 \
                         and obj_line.analytic_account_id.balance<0:
                         raise osv.except_osv(_('Warning !'), 
                                              _('This bank payment is over budget in %s') %obj_line.analytic_account_id.complete_name)
            if move.name =='/':
                new_name = False
            ...
            #Edit 2015-05-25            
            line_expense_obj = self.pool.get('x_expense.recurring.line') 
            if move.x_expense_of :
                for line in move.line_id:
                    if str(line.account_id.code)[0:3] in ('142', '242'): 
                        res = { 'expense_id': move.x_expense_of.id,
                                'date': line.date,
                                'name': line.name,
                        ...
                        line_expense_obj.create(cr, uid, res, context=context)

        cr.execute('UPDATE account_move '\
                   'SET state=%s '\
                   'WHERE id IN %s',
                   ('posted', tuple(valid_moves),))
#         #Edit - Check Asset Deprecated Lines
#         depreciation_lin_obj = self.pool.get('account.asset.depreciation.line')
#         posted_depreciation_line_ids = depreciation_lin_obj.search(cr, uid, [('move_id', 'in', ids), ('move_check', '=', False)])
#         if posted_depreciation_line_ids:
#             cr.execute('UPDATE account_asset_depreciation_line '\
#                        'SET move_check = %s '\
#                        'WHERE id IN %s', (True, tuple(posted_depreciation_line_ids)))
        
        #Edit
        company_currency = self.pool.get('res.users').browse(cr, uid, uid, context=context).company_id.currency_id.id
#         cr.execute('UPDATE account_move_line SET currency_id=%s '\
#                    'WHERE currency_id is null',(company_currency,))
        self.amount_compute(cr, uid, [move.id], context=context)
        return True
    
    #Edit
    def amount_compute(self, cr, uid, ids, context=None):
        for obj_move in self.browse(cr, uid, ids, context=context): 
            cr.execute('SELECT sum(debit) FROM account_move_line WHERE move_id=%s', 
                   (obj_move.id,))
            amount_total = cr.fetchall()[0]
        
            cr.execute('UPDATE account_move SET x_amount_total=%s WHERE id=%s', 
                   (amount_total, obj_move.id,))
        return True
                    
    #Edit    
    def button_taxes(self, cr, uid, ids, context=None):                               
        for obj_move in self.browse(cr, uid, ids, context=context): 
            move_lines = self.pool.get('account.move.line').search(cr, uid, [('move_id','=',obj_move.id)], context=context)            
            
            cr.execute('SELECT DISTINCT b.id FROM account_account b, account_account_type c '\
                    'WHERE c.id=b.user_type and c.code=%s', ('tax',))
            
            if move_lines:
                lines = self.pool.get('account.move.line').browse(cr, uid, move_lines, context=context)
                for line in lines:  
                    if line.tax_code_id.id:      
                        
                        cr.execute('DELETE FROM account_move_line WHERE id=%s AND x_tax_expense != 0',(line.id,))                                  
                        #cr.execute('DELETE FROM account_move_line WHERE id=%s AND x_tax_expense = 1',(line.id,))
                        
                        if line.x_tax_expense == 0:        
                            cr.execute('SELECT b.id FROM account_tax a, account_account b, account_tax_code c ' \
                                           'WHERE a.account_paid_id=b.id and a.tax_code_id=c.id and c.id=%s', (line.tax_code_id.id,))
                            isTaxAccount = cr.fetchall()
                                
                            x_line = line.name
                            if (len(isTaxAccount) > 0):
                                tax_account_id = isTaxAccount[0]
                                cr.execute('SELECT a.amount FROM account_tax a, account_account b, account_tax_code c ' \
                                           'WHERE a.account_paid_id=b.id and a.tax_code_id=c.id and c.id=%s', (line.tax_code_id.id,))
                                tax_rate = cr.fetchone()[0]           
                                tax_amount = round(tax_rate * line.tax_amount)
                                x_line = line.tax_code_id.name
                                analytic_id = False
                            else:
                                tax_account_id = line.account_id.id
                                cr.execute('SELECT a.amount FROM account_tax a, account_tax_code c ' \
                                           'WHERE a.tax_code_id=c.id and c.id=%s', (line.tax_code_id.id,))
                                tax_rate = cr.fetchone()[0]           
                                tax_amount = round(tax_rate * line.tax_amount)  
                                x_line = line.name           
                                analytic_id = line.analytic_account_id and line.analytic_account_id.id              
                                
                            x_tran_date = line.x_tran_date or '1900-01-01'
                            x_date = obj_move.date
                            x_number = line.x_number or ''
                            x_serial = line.x_serial or ''
                            x_invoice_code = line.x_invoice_code or ''
                            x_invoice_sign = line.x_invoice_sign or ''
                            
                            x_project = line.x_project and line.x_project.id or False
                            x_tax_expense = line.id
                            x_product_name = line.x_product_name
                            x_corres_id = line.x_corres_id
                            x_contract_id = line.x_contract_id and line.x_contract_id.id or False
                                
                            #Update x_tax_amount for current line
                            cr.execute('UPDATE account_move_line SET x_tax_amount = %s WHERE id = %s' % (tax_amount, line.id))
                            
                            #Insert tax line into account_move_line
                            move_line_obj = self.pool.get('account.move.line')
                            move_line_obj.create
                            #Edit: make analytic = 4 when analytic is null
                            if not analytic_id:
                                user = self.pool.get('res.users').browse(cr, uid, uid, context=context)
                                analytic_id = user.company_id.analytic_account_id and user.company_id.analytic_account_id.id or False
                            ...
                            if analytic_id:                        
                                if x_project:
                                    if line.partner_id.id:
                                        if x_contract_id:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, partner_id, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, analytic_account_id, x_project, x_tax_expense, x_product_name, x_corres_id, x_contract_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.partner_id.id, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, analytic_id, x_project, x_tax_expense, x_product_name, x_corres_id, x_contract_id))
                                        else:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, partner_id, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, analytic_account_id, x_project, x_tax_expense, x_product_name, x_corres_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.partner_id.id, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, analytic_id, x_project, x_tax_expense, x_product_name, x_corres_id))
                                    else:
                                        if x_contract_id:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, analytic_account_id, x_project, x_tax_expense, x_product_name, x_corres_id, x_contract_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, analytic_id, x_project, x_tax_expense, x_product_name, x_corres_id, x_contract_id))
                                        else:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, analytic_account_id, x_project, x_tax_expense, x_product_name, x_corres_id) VALUES '\
                                    ...
                                    if line.partner_id.id:
                                        if x_contract_id:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, partner_id, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, analytic_account_id, x_tax_expense, x_product_name, x_corres_id, x_contract_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.partner_id.id, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, analytic_id, x_tax_expense, x_product_name, x_corres_id, x_contract_id))
                                        else:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, partner_id, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, analytic_account_id, x_tax_expense, x_product_name, x_corres_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.partner_id.id, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, analytic_id, x_tax_expense, x_product_name, x_corres_id))
                                    else:
                                        if x_contract_id:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, analytic_account_id, x_tax_expense, x_product_name, x_corres_id, x_contract_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                ...
                                if x_project:
                                    if line.partner_id.id:
                                        if x_contract_id:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, partner_id, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, x_project, x_tax_expense, x_product_name, x_corres_id, x_contract_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.partner_id.id, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, x_project, x_tax_expense, x_product_name, x_corres_id, x_contract_id))
                                        else:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, partner_id, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, x_project, x_tax_expense, x_product_name, x_corres_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.partner_id.id, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, x_project, x_tax_expense, x_product_name, x_corres_id))
                                    else:
                                        if x_contract_id:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, x_project, x_tax_expense, x_product_name, x_corres_id, x_contract_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, x_project, x_tax_expense, x_product_name, x_corres_id, x_contract_id))
                                        else:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, x_project, x_tax_expense, x_product_name, x_corres_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, x_project, x_tax_expense, x_product_name, x_corres_id))
                                else:
                                    if line.partner_id.id:
                                        if x_contract_id:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, partner_id, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, x_tax_expense, x_product_name, x_corres_id, x_contract_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.partner_id.id, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, x_tax_expense, x_product_name, x_corres_id, x_contract_id))
                                        else:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, partner_id, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, x_tax_expense, x_product_name, x_corres_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.partner_id.id, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, x_tax_expense, x_product_name, x_corres_id))
                                    else:
                                        if x_contract_id:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, x_tax_expense, x_product_name, x_corres_id, x_contract_id) VALUES '\
                                                       '(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', 
                                                       (line.tax_code_id.id, line.journal_id.id, x_serial, x_invoice_code, x_invoice_sign, line.credit, line.centralisation, line.company_id.id, line.state, tax_amount, line.ref, tax_account_id, line.period_id.id, x_date, line.move_id.id, x_line, tax_amount, x_number, line.amount_currency, x_tran_date, x_tax_expense, x_product_name, x_corres_id, x_contract_id))
                                        else:
                                            cr.execute('INSERT INTO account_move_line(tax_code_id, journal_id, x_serial, x_invoice_code, x_invoice_sign, credit, centralisation, company_id, state, debit, ref, account_id, period_id, date, move_id, name, tax_amount, x_number, amount_currency, x_tran_date, x_tax_expense, x_product_name, x_corres_id) VALUES '\
    ...
    def button_validate(self, cursor, user, ids, context=None):
        for move in self.browse(cursor, user, ids, context=context):
            ...
            #Edit: check expense over budget
            if move.journal_id.type.lower() == 'expense':
                for line in move.line_id:
                    if line.account_id.user_type.name.lower() == 'cash':
                        if line.account_id.balance < 0:
                            raise osv.except_osv(_('Warning !'), _('This expense is over budget') )
        #Edit: validate and close window when validated pop of reduce asset
            if context['active_model'] == 'x_asset.history':                
                asset_histories = self.pool.get('x_asset.history')
                move_idd = move.id
                history_ids = asset_histories.search(cursor, user, [('move_id','=', move.id)], context=context)
                for history_id in history_ids:
                    history = asset_histories.browse(cursor, user, history_id, context=context)
                    if history.code <> move.ref:
                        raise osv.except_osv(_('Invalid Reference !'),_('Reference must be same to "%s"! in Reduce Asset') % (history.code))
                    if history.date <> move.date:
                        raise osv.except_osv(_('Invalid Date !'),_('Date must be same to "%s"! in Reduce Asset') % (history.code))
                    if history.category_id.journal_id.id <> move.journal_id.id:
                        raise osv.except_osv(_('Invalid Journal !'),_('Journal must be same to "%s"! in Reduce Asset') % (history.code))
                self.post(cursor, user, ids, context=context)
    ...
    def button_cancel(self, cr, uid, ids, context=None):
        for move in self.browse(cr, uid, ids, context=context):
            ...
            elif not move.journal_id.update_posted:
                raise osv.except_osv(_('Error !'), _("""You can not modify a posted entry of this journal !\n
                                                        You should set the journal to allow canceling entries if you want to do that."""))                    
             
            if move.x_expense_of:
                line_expense_obj = self.pool.get('x_expense.recurring.line')
                for line in move.line_id:                    
                    if str(line.account_id.code)[0:3] in ('142', '242'): 
                        res = line_expense_obj.search(cr, uid, [('move_line_id', '=', line.id),
                                                                ('expense_id', '=', move.x_expense_of.id)], context=context)
                        if res:
                            line_expense_obj.unlink(cr, uid, res, context=context)
#         if ids:
#             cr.execute('UPDATE account_move '\
#                        'SET state=%s '\
#                        'WHERE id IN %s', ('draft', tuple(ids),))            
#             #Edit - Uncheck Asset Deprecated Lines
#             depreciation_lin_obj = self.pool.get('account.asset.depreciation.line')
#             posted_depreciation_line_ids = depreciation_lin_obj.search(cr, uid, [('move_id', 'in', ids), ('move_check', '=', True)])
#             if posted_depreciation_line_ids:
#                 cr.execute('UPDATE account_asset_depreciation_line '\
#                            'SET move_check = %s '\
#                            'WHERE id IN %s', (False, tuple(posted_depreciation_line_ids)))
    ...
    def onchange_period(self, cr, uid, ids, date, period_id, company_id, context=None):
        res = {}
        period_pool = self.pool.get('account.period')                
        ...
        pids = period_pool.find(cr, uid, dt=date, context=ctx)
        if pids:
            if period_id not in pids:
                res.update({'period_id': False})
                context.update({'period_id': False})
                raise osv.except_osv(_('Warning !'), _('This period is not suitable!'))
            else:                            
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if context is None:
            context = {}
        #Edit
        #Update ref cua lines khi sua ref cua move
        #raise osv.except_osv(_(''), _('Vals: %s') % (vals,))
        situation = self.pool.get('account.journal').search(cr, uid, [('type', '=', 'situation'), ('centralisation','=', True)], context=context)
        if 'ref' in vals:       
            cr.execute('UPDATE account_move_line SET ref = %s WHERE move_id = %s and journal_id not in %s', (vals['ref'], ids[0], tuple(situation)))
        if 'date' in vals:       
            cr.execute('UPDATE account_move_line SET date = %s WHERE move_id = %s  and journal_id not in %s', (vals['date'], ids[0], tuple(situation)))
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        #    if ('ref' in vals) and context['journal_type']=='expense':
        #        line_ids = move_pool.search(cr, uid, [('ref', '=', vals['ref']),('ref','like','PC%')])
        #        line_ids1 = move_pool.search(cr, uid, [('ref', '=', vals['ref']),('ref','like','PT%')])
        #        if line_ids or line_ids1:#(!= '' de loai truong hop duplicate mac dinh origin = ''
        #            raise osv.except_osv(_('Warning !'),_('This referent %s already exit, please check and input again!')%(vals['ref'],))
        ...
        for move in self.browse(cr, uid, ids, context=context):
            if move['state'] != 'draft':
                raise osv.except_osv(_('UserError'),
                        _('You can not delete a posted journal entry "%s"!') %  move['name'])
            ...
            try:
                result = super(account_move, self).unlink(cr, uid, move.id, context)
                if context.get('user_cancel',False):
                    log = logging.getLogger('User Cancel Record')
                    message = cr.dbname + ' Record '+ move.name + ' (Ref: ' +move.ref+ ' TranDate: ' + move.date + ')' +\
                            ' of journal ' + move.journal_id.name+' is canceled by '+user.name
                else:
                    log = logging.getLogger('User Delete Record')
                    message = cr.dbname + ' Record GL '+ move.name + ' (Ref: ' +move.ref+ ' TranDate: ' + move.date + ')' +\
                            ' of journal ' + move.journal_id.name+' is direct deleted by '+user.name
                logfile_name = os.path.join(tools.config['root_path'], "ezfam-delete.log")
                hdlr = logging.FileHandler(logfile_name)
                formatter = logging.Formatter('%(asctime)s %(message)s')
                hdlr.setFormatter(formatter)
                logger.addHandler(hdlr)
                #logger.setLevel(logging.INFO)
                getattr(log,LOG_INFO)(message)  
                logger.removeHandler(hdlr)
                self.log(cr, uid, move.id, message, context=context)             
            except Exception, e:
                raise orm.except_orm(_('Configuration Error!'), str(e))
        return True
    ...
    def validate(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context):    
            #Edit - Check Period
            period_pool = self.pool.get('account.period')        
            pids = period_pool.find(cr, uid, dt=move.date, context=context)
            if pids:   
                if move.period_id.id not in pids:
                    raise osv.except_osv(_('Warning !'), _('This period is not suitable!'))
            
            #Edit - Update x_corres_id (Truong hop Expense)
            for line in move.line_id:
                if not line.x_corres_id:
                    cr.execute('update account_move_line set x_corres_id = move_id where id = %s' % (line.id))
            if move.journal_id.type == 'expense':
                cr.execute('update account_move_line set x_corres_id = move_id where move_id = %s' % (move.id))
            #End
            ...
            if abs(amount) < 10 ** -4 and not journal.centralisation:
...
class account_tax_code(osv.osv):
    ...
    _columns = {
        ...
        #Edit
        'x_tax_rate': fields.char('Tax Rate', size=32),
    }
